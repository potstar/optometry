...
class SightWindow(QtCore.QThread,QDialog):
    ...
    def initUi(self):
        ...
        self.teleSignal.emit('message_start')
        self.sight_ms.initUi('测试右眼，请遮住左眼')
        #threading.Thread(target=self.teleThread).start()
        #print(threading.Thread.name)
        #print('ka')
        self.teleSignal.emit('message_stop')
        self.teleSignal.emit('sight_start')
        self.form_sight.exec_()
        self.teleSignal.emit('sight_stop')
    ...
    def endTest(self):
        if self.testObject != '左':
            ...
            self.teleSignal.emit(str(self.testGradLeft)+'_'+str(self.testGradRight)+'_svalue')
            #self.testData()
            self.sleep(1)
